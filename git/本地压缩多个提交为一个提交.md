...
git log这里的 N 就是你想要合并的提交的数量，从最近的一次提交往前数。例如，这是一个假象的从

 中拉取的提交列表，我当前正在修改 feature Z：
...
我有数不清的提交要压缩，必须一个一个的数吗？因为我想将最后的 7 次提交合并为一次，所以 `d94e78 Prepare the workbench for feature X` 就是第 7 次提交。

`git rebase --interactive HEAD~[N]` 命令的缺点就是你必须一个一个的数出准确的提交次数。幸运的是这里还有另一种方式：git rebase --interactive [commit-hash]
...
你可以通过将任务 `pick` 修改为 `squash` （或者简写为 `s` ，评论中有提示）来将提交标记为 squashable 。最后的结果就是：在提交列表的底部有一个简短的注释（示例中忽略了），提示了所有的操作选项。你可以在交互式 rebase 中进行各种操作，我们现在只进行一些基本的操作。我们的任务是将所有的提交注释为  **squashable** ，除了第一个（最早的）提交：它将被用作起始点。
...
通常我不喜欢保留这些信息，所以我会清楚默认消息，然后重新注释，例如 `Implemented feature Z`。

参考：https://kinboyw.github.io/2019/04/09/Git-%E5%8E%8B%E7%BC%A9%E5%A4%9A%E4%B8%AAcommit%E4%B8%BA%E5%8D%95%E4%B8%AAcommit/
